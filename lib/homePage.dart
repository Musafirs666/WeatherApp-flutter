...
import 'package:weather_flutterapp/WeekWeatherPage.dart';
...
import 'package:weather_flutterapp/locationPage.dart';
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  String _choosedCity = 'London';
  int _unixTimestamp = DateTime.now().millisecondsSinceEpoch ~/ 1000;

  @override
  void initState() {
    super.initState();
    // Memanggil fungsi untuk mengambil data dari API saat widget pertama kali dibuat
    fetchData(_choosedCity);
  }
  ...
  Future<void> fetchData(chossedCity) async {
    ...
    final response = await http.get(Uri.parse(
        '${dotenv.env['BASE_URL']}?key=${dotenv.env['API_KEY']}&q=${city}&days=1&aqi=no&alerts=no&unixdt=${_unixTimestamp}'));
    ...
    if (response.statusCode == 200) {
      setState(() {
        ...
        _currentHour = currentHour.toString().padLeft(2, '0');
        ...
        String conditionText =
            _currentForecastData['current']['condition']['text'].toString();
                                  ...
                                  onPressed: () {
                                    Navigator.push(
                                        ...
                                        MaterialPageRoute(
                                            builder: (context) =>
                                                WeekWeatherPage(
                                                  choosedCity: _choosedCity,
                                                  todayForecastData:
                                                      _todayForecastData,
                                                  currentForecastData:
                                                      _currentForecastData,
                                                  currentHour: _currentHour,
                                                  formattedCondition: _formattedCondition,
